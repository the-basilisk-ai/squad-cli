...
husky pre-commit hook scans staged changes. Same setup in `squidge` and `squad-mcp`.
...
`gitleaks git --log-opts="--all"` with gitleaks 8.30.1, every ref up to `5ab5bb95`,
2026-08-02. No findings, so `.gitleaksignore` is empty.
...
giving your name, the date and the reasoning. Push it in the pull
request the finding blocked, so someone other than you reviews the justification.
